...
load_movies <- function() {
  ...
  for (movie_name in names(movies)) {
    ...
    data <- list()
    tryCatch({
      response <- GET(paste0(config$api_base_url, "movie/", id, "?api_key=", config$api_key))
      data <- content(response, as = "parsed")
    }, error = function(e) {
      cat("Error fetching movie details:\n", e$message, "\n")
    })
    
    movie$poster_path <- paste0(config$image_base_url, data$poster_path)
    ...
    tryCatch({
      response2 <- GET(paste0(config$api_base_url, "movie/", id, "/watch/providers?api_key=", config$api_key))
    ...
    tryCatch({
      response3 <- GET(paste0(config$api_base_url, "movie/", id, "/credits?api_key=", config$api_key))
    ...
    if (!is.null(credits$cast)) {
      for (cast_member in credits$cast) {
        ...
        movie$cast[[cast_name]] <- list(
          ...
          profile_path = ifelse(is.null(cast_member$profile_path), movie$poster_path, paste0(config$image_base_url, cast_member$profile_path))
        )
      }
    }
